...
1-Wasserstein is nothing but the worst-case discrepency / deviation between moments with respect to the given likelihoods. The candidate functions for computing the moments come from the special set: 1-Lipschitz functions. We choose that 1-Lipschitz function that induces largest deviation between the moments with respect to the given likelihoods. More informally, among few witnesses $\{r \, | \, \|r\|_d \leq 1\}$, the one that provides the most significant evidence that the likelihoods are different is considered. And, the corresponding deviation is 1-Wasserstein distance.
...
We can solve this optimization problem, which is of the form $\min_{\theta} L(\theta)$, using SGD. To find the gradient, $\nabla_{\theta} L(\theta)$, we need to solve the inner maximization (optimal transport) problem, which can be solved using the Danskin's theorem. Given a value of $\theta$, we solve the inner maximization problem, i.e., it is the same as computing the 1-Wasserstein distance between $p$ and $p^*$. We compute this distance at every iteration of SGD.
...
When $c(x,y) = -x^\top y$, which is a normalized version of the squared Euclidean distance between $x$ and $y$ as
$$
\frac{1}{2}\|x-y\|_2^2 = \frac{1}{2}\|x\|^2 + \frac{1}{2}\|y\|^2 - x^\top y
$$
Then, the optimization problem in the constraint becomes:
$$
\begin{align*}
-\max_x & \{x^\top y - (- r(x))\} \,\, \forall y \\
& \equiv -r^*(y)
\end{align*}
$$

This expression gives the definition of the Fenchel conjugate, that is, it is the expression for conjugate of $-r(x)$. Let's denote it by $-r^*(y)$. We know that $\bar{r}(y) \equiv -r^*(y)$ is a **convex** function.
